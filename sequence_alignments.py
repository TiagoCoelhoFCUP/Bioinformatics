...
def identity(seq1, seq2, alphabet):
    print("\n\ntest identity\n")
    '''calculate the identity score between seq1 and seq2 '''
    match , missmatch = 1 , 0
    gap = 0
    sm = create_submat (match, missmatch, alphabet)
    res = needleman_Wunsch (seq1, seq2, sm, gap)[0]
    res = res[len(seq1)][len(seq2)]
    res = res/max(len(seq1),len(seq2))
...
def test_prot():
    ...
    r = -1
    print("score of the alignment: ", score_align (seq1, seq2, sm, g))
    print("score of the alignment using affine gap: ",score_affinegap (seq1, seq2, sm, g, r))
...
def test_DNA_GlobalAlign():
    ...
    sm = create_submat (match, missmatch, "ACT")
    # solve the NW algorithm with gap p
    res = needleman_Wunsch (seq1, seq2, sm, gap)
    # obtain the score of the alignment: using matrix cells and score alignment function
    S = res[0]
    T = res[1]
    print_mat(S)
    print_mat(T)
    print("Score of DNA Global alignment:", S[len(seq1)][len(seq2)])
...
def test_Prot_LocalAlign():
    ...
    sm1 = read_submat_file("blosum62.mat")
    seq1 = "ANDDR"
    seq2 = "AARRD"
    match = 3
    missmatch = -1
    gap = -3
    sm2 = create_submat (match, missmatch, "ANDR")
    res1 , res2  = smith_Waterman(seq1, seq2, sm1, gap) , smith_Waterman(seq1, seq2, sm2, gap)
    S1 , S2 = res1[0] , res2[0]
    T1 , T2 = res1[1] , res2[1]
    
    
    
    alinL1 , alinL2 = recover_align_local(S1, T1, seq1, seq2) , recover_align_local(S2, T2, seq1, seq2)
    
    
    i1, j1  = max_mat(S1)
    i2, j2  = max_mat(S2)
    best_score1 , best_score2 = S1[i1][j1] ,  S2[i2][j2]
    print("Score of optimal alignment with blosum:", res1[2])
    print_mat(S1)
    print_mat(T1)
    print("Alignment with blosum:")
    print(alinL1[0])
    print(alinL1[1])
    print ("best score with blosum: " + str(best_score1))
    print("Score of optimal alignment with sm of the alphabet:", res2[2])
    print_mat(S2)
    print_mat(T2)
    print("Alignment  with sm of the alphabet:")
    print(alinL2[0])
    print(alinL2[1])
    print ("best score with sm of the alphabet: " + str(best_score2))
